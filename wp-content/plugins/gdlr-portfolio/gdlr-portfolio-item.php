<?php
	...
	*/
	...
	if( !function_exists('gdlr_print_portfolio_item') ){
		function gdlr_print_portfolio_item( $settings = array() ){
			...
			if( $settings['portfolio-style'] == 'classic-portfolio' || 
				...
				$ret .= gdlr_get_classic2_portfolio($query, $settings['portfolio-size'], 
							$settings['thumbnail-size'], $settings['portfolio-layout'] );
	...
	if( !function_exists('gdlr_get_portfolio_info') ){
		function gdlr_get_portfolio_info( $array = array(), $option = array(), $wrapper = true ){
			...
			foreach($array as $post_info){	
				switch( $post_info ){
					case 'time':
						if(empty($option['time'])) break;
					
						$ret .= '<div class="portfolio-info portfolio-time">';
						$ret .= '<span class="info-head gdlr-title">' . __('Time', 'gdlr-portfolio') . ' </span>';
						$ret .= $option['time'];						
						$ret .= '</div>';						
					
						break;	
					case 'staff':
						if(empty($option['staff'])) break;
					
						$ret .= '<div class="portfolio-info portfolio-staff">';
						$ret .= '<span class="info-head gdlr-title">' . __('Staff', 'gdlr-portfolio') . ' </span>';
						$ret .= $option['staff'];						
						$ret .= '</div>';						

						break;	
					case 'price':
						if(empty($option['price'])) break;
					
						$ret .= '<div class="portfolio-info portfolio-price">';
						$ret .= '<span class="info-head gdlr-title">' . __('Price', 'gdlr-portfolio') . ' </span>';
						$ret .= $option['price'];					
						$ret .= '</div>';						
					
						break;	
					case 'brochure':
						if(empty($option['brochure'])) break;
					
						$ret .= '<div class="portfolio-info portfolio-brochure">';
						$ret .= '<i class="icon-file-text fa fa-file-pdf-o" ></i>';
						$ret .= '<a href="' . $option['brochure'] . '">' . __('Download Brochure', 'gdlr-portfolio') . ' </a>';			
						$ret .= '</div>';						
				...
				case 'feature-image':
					...
					if( !empty($image_id) ){
						if( $modern_style ){
							...
							$ret .= '</a>';	
							$ret .= '<div class="gdlr-modern-portfolio-title"><a href="' . get_permalink() . '" >' . get_the_title() . '</a></div>';
	...
	if( !function_exists('gdlr_get_medium_portfolio') ){
		function gdlr_get_medium_portfolio($query, $thumbnail_size){
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= gdlr_get_portfolio_info(array('time', 'staff', 'price', 'tag', 'brochure'), $port_option, false); 
	...
	if( !function_exists('gdlr_get_classic_portfolio') ){
		function gdlr_get_classic_portfolio($query, $size, $thumbnail_size, $layout = 'fitRows'){
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<h3 class="gdlra-skin-box portfolio-title ' . (empty($port_option['price'])? '': 'with-price') . '">';
				$ret .= '<a ' . gdlr_get_portfolio_thumbnail_link($port_option, 'title') . ' >' . get_the_title() . '</a>';
				
				if( !empty($port_option['price']) ){
					$ret .= '<span class="gdlr-port-price">' . $port_option['price'] . '</span>';
				}
				$ret .= '</h3>';
	...
	if( !function_exists('gdlr_get_classic2_portfolio') ){
		function gdlr_get_classic2_portfolio($query, $size, $thumbnail_size, $layout = 'fitRows'){
			if($layout == 'carousel'){ 
				return gdlr_get_classic2_carousel_portfolio($query, $size, $thumbnail_size); 
			}		
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<h3 class="portfolio-title ' . (empty($port_option['price'])? '': 'with-price') . '">';
				$ret .= '<a ' . gdlr_get_portfolio_thumbnail_link($port_option, 'title') . ' >' . get_the_title() . '</a>';
	...
	if( !function_exists('gdlr_get_classic2_carousel_portfolio') ){
		function gdlr_get_classic2_carousel_portfolio($query, $size, $thumbnail_size){	
			global $post;
