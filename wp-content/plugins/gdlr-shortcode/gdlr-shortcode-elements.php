<?php
	...
	function gdlr_testimonial_shortcode( $atts, $content = null ){
		...
		$settings['title'] = $title;
		$settings['title-type'] = 'left-small';
	...
	function gdlr_personnel_shortcode( $atts, $content = null ){
		extract( shortcode_atts(array('columns'=>3, 'type'=>'fitRows', 'category'=>'',
			'thumbnail_size'=>'', 'num_fetch'=>6), $atts) );

		$settings = array();
		
		do_shortcode($content);
		$settings['item-size'] = $columns;
		$settings['personnel-layout'] = $type;
		$settings['category'] = $category;
		$settings['thumbnail-size'] = $thumbnail_size;
		$settings['num-fetch'] = $num_fetch;

		if( function_exists('gdlr_print_personnel_item') ){
			return '<div class="gdlr-shortcode-wrapper">' . gdlr_print_personnel_item($settings) . '</div>';
		}
		return '';
	}	
