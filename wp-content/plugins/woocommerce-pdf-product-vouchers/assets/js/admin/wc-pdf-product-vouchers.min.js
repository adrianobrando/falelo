  ...
  jQuery(function($) {
    ...
    if ('wc_voucher' === pagenow || 'edit-wc_voucher' === pagenow) {
      ...
      $('.page-title-action, .add-new-h2').on('click', function(event) {
        ...
        content: function() {
          var form, product_search_input;
          if (pdf_vouchers.is_wc_version_gte_3_0) {
            product_search_input = '<p><select class="wc-product-search" name="product" id="search_product_id" style="width:100%" data-exclude="wc_pdf_product_vouchers_non_voucher_products" data-placeholder="' + pdf_vouchers.i18n.search_for_product + '" data-allow_clear="true" ></select></p>';
          } else {
            product_search_input = '<p><input type="hidden" class="wc-product-search" name="product" id="search_product_id" style="width:100%" data-exclude="wc_pdf_product_vouchers_non_voucher_products" data-placeholder="' + pdf_vouchers.i18n.search_for_product + '" data-allow_clear="true" /></p>';
          }
          form = '<form method="get" action="' + pdf_vouchers.new_voucher_url + '"> <input type="hidden" name="post_type" value="wc_voucher" /> <h3 class="wc-pdf-product-vouchers-voucher-create-voucher">' + pdf_vouchers.i18n.select_product + '</h3>' + product_search_input + '</form>';
          return form;
        },
        buttons: function() {
    ...
    if ('wc_voucher' === pagenow) {
      ...
      $(document.body).on('wc_backbone_modal_loaded', function(e, target) {
        ...
        return $(document.body).on('change.edit-product-modal', '#voucher_product_id', function(event) {
          var data;
          $('#voucher_product_price').closest('p').block(blockui_options);
          data = {
          ...
          return $.get(wc_pdf_product_vouchers_admin.ajax_url, data, function(response) {
            ...
            if (response.success && response.data) {
              ...
              return $('#voucher_product_price').val(formatted_price).closest('p').unblock();
            }
          });
        });
        ...
        return validate_redemption_value($(this), $('#voucher_remaining_value').val());
      ...
      $('#customer_id').on('change', load_customer_details);
      ...
      $('#wc-pdf-product-vouchers-voucher-balance').on('click', '.js-edit-voucher-product', function(event) {
        ...
        data = {
          ...
          security: wc_pdf_product_vouchers_admin.voucher_balance_nonce,
          voucher_id: $('#post_ID').val(),
          data: $('tbody#voucher-redemptions :input[name]').serialize()
        };
        $('#wc-pdf-product-vouchers-voucher-balance').block(blockui_options);
        return $.post(wc_pdf_product_vouchers_admin.ajax_url, data, handle_balance_response);
      });
    ...
    if ('edit-wc_voucher' === pagenow) {
      $('.redeem').on('click', function(event) {
        ...
        var $el, $pointer, remaining_value;
        $el = $(this);
        $pointer = $el.data('wp-pointer').pointer;
        remaining_value = $el.closest('.voucher_actions').find('.voucher-remaining-value').val();
      ...
      $(document.body).on('keyup', '.js-wc-pdf-vouchers-redeem-amount', function() {
        return validate_redemption_value($(this), $(this).closest('.redeem-voucher-pointer').data('voucher-remaining-value'));
      }).on('blur', '.js-wc-pdf-vouchers-redeem-amount', function() {
        return $('.wc_error_tip').fadeOut('100', function() {
          return $(this).remove();
        });
    ...
    /**
    	 ...
    	 *
    	 * @since 3.0.0
    	 * @param {object} $el Input element to validate
    	 * @param {float} remaining_value Remaining voucher value
    	 * @return bool True if valid, false otherwise
     */
    validate_redemption_value = function($el, remaining_value) {
      var amount, newvalue, regex, value;
      regex = new RegExp('[^0-9\\' + woocommerce_admin.mon_decimal_point + ']+', 'gi');
      value = $el.val();
      newvalue = value.replace(regex, '');
      if (value !== newvalue) {
        $(document.body).triggerHandler('wc_add_error_tip', [$el, 'i18n_mon_decimal_error']);
        return false;
      } else {
        $(document.body).triggerHandler('wc_remove_error_tip', [$el, 'i18n_mon_decimal_error']);
      }
      amount = parseFloat(window.accounting.unformat(value, woocommerce_admin.mon_decimal_point));
      if (!amount || amount < 0) {
        $(document.body).triggerHandler('wc_add_error_tip', [$el, 'pdf_product_vouchers_i18n_amount_greater_than_zero_error']);
        return false;
      } else {
        $(document.body).triggerHandler('wc_remove_error_tip', [$el, 'pdf_product_vouchers_i18n_amount_greater_than_zero_error']);
      }
      if (amount > remaining_value) {
        $(document.body).triggerHandler('wc_add_error_tip', [$el, 'pdf_product_vouchers_i18n_amount_less_or_equal_to_remaining_error']);
        return false;
      } else {
        $(document.body).triggerHandler('wc_remove_error_tip', [$el, 'pdf_product_vouchers_i18n_amount_less_or_equal_to_remaining_error']);
      }
      return true;
    };
    ...
    handle_redeem_form_submit = function($pointer) {
      var $el, $form, data, url;
      $el = $pointer.find('input[name="amount"]');
      if (!validate_redemption_value($el, $el.closest('.redeem-voucher-pointer').data('voucher-remaining-value'))) {
        return $el.focus();
      }
