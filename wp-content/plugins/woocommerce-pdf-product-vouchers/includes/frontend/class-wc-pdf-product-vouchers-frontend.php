...
class WC_PDF_Product_Vouchers_Frontend {
	...
	public function __construct() {
		...
		add_action( 'wp_head', array( $this, 'clean_voucher_template_actions' ), -1 );
		add_action( 'wp_enqueue_scripts', array( $this, 'clean_voucher_template_styles_and_scripts' ), 999 );
	...
	public function enqueue_scripts() {
		...
		if ( is_singular() ) {
			...
			if ( in_array( true, $product_has_voucher_template, true ) ) {
				...
				if ( in_array( true, $product_is_variable, true ) ) {
	...
	public function clean_voucher_template_actions() {
		...
		if ( is_singular( 'wc_voucher_template' ) || is_singular( 'wc_voucher' ) ) {
			...
			if ( ! empty( $wp_filter['wp_head'] ) ) {
				...
				foreach ( $wp_filter['wp_head'] as $priority => $actions ) {

					foreach ( $actions as $key => $action  ) {

						// remove action if it's not allowed for voucher templates
						if ( ! $this->allow_voucher_template_wp_head_action( $action, $priority ) ) {
							$actions_to_remove[] = array(
			...
			// important note: this needs to have priority 1000
			remove_action( 'wp_footer', 'wp_admin_bar_render', 1000 );

			// actions cannot be removed while iterating through wp_filter above, it must
			// be done afterwards
			foreach ( $actions_to_remove as $action ) {
				remove_action( 'wp_head', $action['callback'], $action['priority'] );
			}
		}
	}
	...
	/**
	 * Checks whether to allow an action to run in wp_head or not for voucher templates
	 *
	 * By default, only actions added by this class and WP_Cusomize_Manager are allowed.
	 *
	 * @since 3.0.0
	 ...
	 */
	private function allow_voucher_template_wp_head_action( $action, $priority ) {
		...
		/**
		 * Filters whether to allow an action to run in wp_head or not for voucher templates
		 *
		 * @since 3.0.0
		 * @param bool $allow whether to allow an action to run or not
		 * @param array $action associative array containing infor about the action from $wp_filters
		 * @param int $priority action priority
		 */
		return apply_filters( 'wc_pdf_product_vouchers_allow_wc_voucher_template_wp_head_action', $allow, $action, $priority );
	}
