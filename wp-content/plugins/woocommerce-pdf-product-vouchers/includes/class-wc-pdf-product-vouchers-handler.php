...
class WC_PDF_Product_Vouchers_Handler {
	...
	public function activate_vouchers( $order_id, $force_generate_pdf = false ) {
		...
		foreach ( WC_PDF_Product_Vouchers_Order::get_vouchers( $order ) as $voucher ) {
			...
			try {
				...
				$order->add_order_note( sprintf( __( 'PDF Product Vouchers: unable to generate PDF for voucher %1$s: %2$s', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), $exception->getMessage() ) );
			}
		}
	}
	...
	public function order_item_meta_start( $item_id, $item, WC_Order $order ) {
		...
		echo esc_html( _n( 'Voucher:', 'Vouchers:', count( $links ), 'woocommerce-pdf-product-vouchers' ) ) . '</strong> ' . implode( ', ', $links );
	...
	public function add_voucher_generation_test_tool( $tools ) {
		...
		$tools['wc_pdf_product_vouchers_test_voucher_generation'] = array(
			'name'     => __( 'PDF Vouchers', 'woocommerce-pdf-product-vouchers' ),
			'button'   => __( 'Test voucher generation', 'woocommerce-pdf-product-vouchers' ),
			'desc'     => __( 'This tool will test whether your server is capable of generating PDF vouchers.', 'woocommerce-pdf-product-vouchers' ),
	...
	public function test_voucher_generation() {
		...
		if ( ! $success && is_admin() ) {
			echo '<div class="error inline"><p>' . esc_html__( 'Could not generate test voucher. Please ensure your server has loopback connections enabled.', 'woocommerce-pdf-product-vouchers' ) . '</p></div>';
		}
