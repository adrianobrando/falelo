...
class WC_Voucher extends WC_Voucher_Base {
	...
	public function __construct( $id ) {
		...
		if ( $this->post ) {
			...
			$this->customer_id = $this->post->post_author;
		}
	}
	...
	public function redeem( $amount = null, $args = array() ) {
		...
		if ( ! $this->get_remaining_value() ) {
			throw new SV_WC_Plugin_Exception( __( 'No remaining value', 'wc-pdf-product-vouchers' ) );
		}

		if ( ! $amount || $amount < 0 ) {
			throw new SV_WC_Plugin_Exception( __( 'Redemption amount must be a positive number', 'wc-pdf-product-vouchers' ) );
		}

		if ( $this->get_remaining_value() < $amount ) {
			throw new SV_WC_Plugin_Exception( __( 'Redemption amount is greater than remaining voucher value', 'wc-pdf-product-vouchers' ) );
		}
	...
	public function set_redemptions( $redemptions = array() ) {
		...
		foreach ( $redemptions as $i => $redemption ) {
			...
			if ( ! $amount || $amount < 0 ) {
				throw new SV_WC_Plugin_Exception( sprintf( __( 'Invalid redemption amount for redemption #%d.', 'woocommerce-pdf-product-vouchers' ), $i + 1 ) );
			}
	...
	public function get_field_value( $field_id ) {
		...
		switch ( $field_id ) {
			...
			case 'logo':
				$value = $this->get_template()->get_logo_id();
			break;
