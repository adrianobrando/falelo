...
class WC_PDF_Product_Vouchers_Meta_Box_Voucher_Data {
	...
	public function save( $post_id, WP_Post $post ) {
		...
		// update customer
		if ( isset( $_POST['customer_id'] ) && $_POST['customer_id'] !== $post->post_author ) {
			$wpdb->query( $wpdb->prepare( "UPDATE $wpdb->posts SET post_author = %d WHERE ID = %d", $_POST['customer_id'], $post_id ) );
		}
		...
		if ( ! $voucher->get_voucher_number() ) {
			$voucher_number = wc_pdf_product_vouchers_generate_voucher_number( $voucher->get_order_id() );
		}
