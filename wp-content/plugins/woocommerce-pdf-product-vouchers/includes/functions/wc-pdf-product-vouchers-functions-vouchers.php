...
function wc_pdf_product_vouchers_create_voucher( $args = array() ) {
	...
	if ( (int) $args['voucher_image_id'] > 0 ) {
		...
		if ( ! is_callable( array( $voucher->get_template(), 'get_image_id' ) ) ) {
	...
	if ( ! $updating ) {
		...
		if ( null === $args['product_price'] ) {
			...
			if ( ( $order = $voucher->get_order() ) && ( $item = $voucher->get_order_item() ) ) {

				$args['product_price'] = $order->get_item_subtotal( $item );
				$args['product_tax']   = $order->get_item_tax( $item );

			} elseif ( $product = $this->get_product() ) {

				$price_incl_tax = SV_WC_Product_Compatibility::wc_get_price_including_tax( $product );
				$price_excl_tax = SV_WC_Product_Compatibility::wc_get_price_excluding_tax( $product );
				$tax            = $price_incl_tax - $price_excl_tax;

				$args['product_price'] = $price_excl_tax;
		...
		update_post_meta( $voucher->get_id(), '_remaining_value',  $args['product_price'] * $args['product_quantity'] );
	}
	 ...
	 */
	do_action( 'wc_pdf_product_vouchers_voucher_created', $voucher );
