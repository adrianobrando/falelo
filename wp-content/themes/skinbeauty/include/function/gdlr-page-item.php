<?php
	...
	*/
	...
	if( !function_exists('gdlr_get_item_title') ){
		function gdlr_get_item_title( $atts ){
			...
			if( !empty($atts['title-type']) && $atts['title-type'] != 'none' && 
				...
				$item_class  = (!empty($atts['title-type']))? 'pos-' . $atts['title-type']: '';
				
				$ret .= '<div class="gdlr-item-title-container container ' . (empty($atts['carousel'])? '': 'gdlr-nav-container') . '">';
				...
				if( !empty($atts['carousel']) ){
					...
					$ret .= '<i class="icon-angle-left gdlr-flex-prev gdlra-skin-box"></i>';
					$ret .= '<i class="icon-angle-right gdlr-flex-next gdlra-skin-box"></i>';
					$ret .= '</div>';
				}
				...
				$ret .= '</div>'; // gdlr-item-title-container
			}
			return $ret;
		}
	}		
	...
	// quote item
	if( !function_exists('gdlr_get_quote_item') ){
		function gdlr_get_quote_item( $settings ){
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
			...
			$ret  = '<div class="gdlr-item gdlr-quote-item" ' . $item_id . $margin_style . '>';
			$ret .= '<div class="gdlr-quote-item-content-wrapper">';
			$ret .= '<div class="gdlr-quote-item-content">' . gdlr_content_filter($settings['content']) . '</div>';
			$ret .= '<div class="gdlr-quote-item-author">' . gdlr_text_filter($settings['author']) . '</div>';
			$ret .= '<div class="gdlr-quote-item-position">' . gdlr_text_filter($settings['position']) . '</div>';	
			$ret .= '</div>'; // quote-item
			$ret .= '</div>'; // quote-item-content-wrapper
	...
	if( !function_exists('gdlr_get_column_service_item') ){
		function gdlr_get_column_service_item( $settings ){
			...
			$featured = ($settings['style'] == 'type-3-featured')? 'gdlr-featured-column-service': '';
			$settings['style'] = empty($settings['style'])? 'type-1': str_replace('-featured', '', $settings['style']);
			
			if( !empty($featured) ){
				$ret  = '<div class="' . $featured . '" >';
				$ret .= '<div class="gdlr-feature-overlay-bg" style="background: ' . $settings['featured-background'] . ';" ></div>';
				$ret .= '<div class="gdlr-feature-overlay-inner-bg" style="background: ' . $settings['featured-background'] . ';" ></div>';
			}else{
				$ret  = '';
			}
			...
			if( !empty($settings['learn-more-text']) && !empty($settings['learn-more-link']) ){
				$ret .= '<a class="column-service-read-more" href="' . $settings['learn-more-link'] . '" >' . $settings['learn-more-text'] . '</a>';	
			}
		...
		function gdlr_get_price_table_item( $settings ){
			...
			foreach( $list as $tab ){ 
				...
				if(!empty($tab['gdl-tab-link'])){
					...
					$ret .= '<a class="gdlr-button without-border" href="' . esc_url($tab['gdl-tab-link']) . '">' . __('Abbonati', 'gdlr_translate') . '</a>';
					$ret .= '</div>';
				}
	...
	if( !function_exists('gdlr_get_static_testimonial_item') ){
		function gdlr_get_static_testimonial_item( $settings ){	
			...
			foreach( $list as $tab ){ 
				...
				$ret .= '<div class="gdlr-item gdlr-testimonial-item ' . $settings['testimonial-style'] . ' ' . (($settings['testimonial-style'] == 'widget-style')? 'gdlr-title-font':'') . '">';
	...
	if( !function_exists('gdlr_get_carousel_testimonial_item') ){
		function gdlr_get_carousel_testimonial_item( $settings ){	
			...
			$ret .= gdlr_get_item_title($settings);							
			$ret .= '<div class="gdlr-item gdlr-testimonial-item carousel ' . $settings['testimonial-style'] . ' ' . (($settings['testimonial-style'] == 'widget-style')? 'gdlr-title-font':'') . '">';
			$ret .= '<div class="gdlr-ux gdlr-testimonial-ux">';
