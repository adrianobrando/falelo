	...
	*/	
	...
	if( !function_exists('gdlr_get_twitter_item') ){
		function gdlr_get_twitter_item( $settings ){
			...
			$ret  = gdlr_get_item_title($settings);	
			$ret .= '<div class="gdlr-item gdlr-twitter-item-wrapper" >';	
			...
			foreach( $tweets_data as $tweet_data ){
				$ret .= '<li class="gdlr-title-font">' . utf8_decode($tweet_data) . '</li>';
			}
			$ret .= '</ul>';			
			$ret .= '</div>'; // flexslider
			$ret .= '</div>';
			
			$ret .= '<div class="gdlr-nav-container" >';
			$ret .= '<i class="fa fa-angle-left icon-angle-left gdlr-flex-prev" ></i>';
			$ret .= '<a href="https://twitter.com/' . $settings['twitter-name'] . '" target="_blank" >';
			$ret .= '<i class="fa fa-twitter icon-twitter" ></i>';
			$ret .= '</a>';
			$ret .= '<i class="fa fa-angle-right icon-angle-right gdlr-flex-next" ></i>';
			$ret .= '</div>';
			$ret .= '</div>'; // gdlr-twitter-item-wrapper
			return $ret;
		}
	}	
